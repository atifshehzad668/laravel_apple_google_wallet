...
class EventTicketPass
{
    ...
    public function createClass(string $issuerId, string $classSuffix)
    {
        ...
        try {
            ...
            if (empty($ex->getErrors()) || $ex->getErrors()[0]['reason'] != 'classNotFound') {
                ...
                Log::error('Google Wallet Class get failed: ' . $ex->getMessage());
                return "{$issuerId}.{$classSuffix}";
            }
        }
  ...
  // [START createObject]
    ...
    public function createObject(string $issuerId, string $classSuffix, string $objectSuffix, array $data = [])
    {
        ...
        try {
            $existing = $this->service->eventticketobject->get("{$issuerId}.{$objectSuffix}");
            return $existing;
        } catch (\Google\Service\Exception $ex) {
            if (empty($ex->getErrors()) || $ex->getErrors()[0]['reason'] != 'resourceNotFound') {
                // Something else went wrong...
                Log::error('Google Wallet Object get failed: ' . $ex->getMessage());
                return null;
            }
        }
        ...
        $newObject = new EventTicketObject([
            ...
            'heroImage' => new Image([
                'sourceUri' => new ImageUri([
                    'uri' => $data['hero_image_url'] ?? 'https://farm4.staticflickr.com/3723/11177041115_6e6a3b6f49_o.jpg'
                ]),
            ...
            'textModulesData' => [
                new TextModuleData([
                    'header' => $data['text_module_header'] ?? 'Text module header',
                    'body' => $data['text_module_body'] ?? 'Text module body',
                    'id' => 'TEXT_MODULE_ID'
                ])
            ],
            'linksModuleData' => new LinksModuleData([
                'uris' => [
                    new Uri([
                        'uri' => $data['homepage_url'] ?? 'http://maps.google.com/',
            ...
            'imageModulesData' => [
                new ImageModuleData([
                    'mainImage' => new Image([
                        'sourceUri' => new ImageUri([
                            'uri' => $data['main_image_url'] ?? 'http://farm4.staticflickr.com/3738/12440799783_3dc3c20606_b.jpg'
                        ]),
                        'contentDescription' => new LocalizedString([
                            'defaultValue' => new TranslatedString([
                                'language' => 'en-US',
            ...
            'barcode' => new Barcode([
                'type' => $data['barcode_type'] ?? 'QR_CODE',
                'value' => $data['barcode_value'] ?? 'QR code value'
            ]),
            'locations' => [
                new LatLongPoint([
                    'latitude' => $data['latitude'] ?? 37.424015499999996,
                    'longitude' =>  $data['longitude'] ?? -122.09259560000001
                ])
            ],
            ...
            'ticketHolderName' => $data['name'] ?? 'Ticket holder name',
            'ticketNumber' => $data['ticket_number'] ?? 'Ticket number'
        ]);
  ...
  // [START updateObject]
    ...
    public function updateObject(string $issuerId, string $objectSuffix)
    {
        ...
        try {
            ...
            if (!empty($ex->getErrors()) && $ex->getErrors()[0]['reason'] == 'resourceNotFound') {
                Log::warning("Object {$issuerId}.{$objectSuffix} not found for update.");
                return "{$issuerId}.{$objectSuffix}";
            } else {
                Log::error('Google Wallet Object get failed for update: ' . $ex->getMessage());
                return "{$issuerId}.{$objectSuffix}";
            }
        }
  ...
  // [START patchObject]
    ...
    public function patchObject(string $issuerId, string $objectSuffix)
    {
        ...
        try {
            ...
            if (!empty($ex->getErrors()) && $ex->getErrors()[0]['reason'] == 'resourceNotFound') {
                Log::warning("Object {$issuerId}.{$objectSuffix} not found for patching.");
                return "{$issuerId}.{$objectSuffix}";
            } else {
                Log::error('Google Wallet Object get failed for patch: ' . $ex->getMessage());
                return "{$issuerId}.{$objectSuffix}";
            }
        }
  ...
  // [START expireObject]
    ...
    public function expireObject(string $issuerId, string $objectSuffix)
    {
        ...
        try {
            ...
            if (!empty($ex->getErrors()) && $ex->getErrors()[0]['reason'] == 'resourceNotFound') {
                Log::warning("Object {$issuerId}.{$objectSuffix} not found for expiration.");
                return "{$issuerId}.{$objectSuffix}";
            } else {
                Log::error('Google Wallet Object get failed for expiration: ' . $ex->getMessage());
                return "{$issuerId}.{$objectSuffix}";
            }
        }

        // Patch the object, setting the pass as expired
        $patchBody = new \Google\Service\Walletobjects\EventTicketObject([
            'state' => 'EXPIRED'
        ]);
    ...
    public function addObjectMessage(string $issuerId, string $objectSuffix, string $header, string $body)
    {
        ...
        try {
            ...
            if (!empty($ex->getErrors()) && $ex->getErrors()[0]['reason'] == 'resourceNotFound') {
                Log::warning("Object {$issuerId}.{$objectSuffix} not found for messaging.");
                return "{$issuerId}.{$objectSuffix}";
            } else {
                Log::error('Google Wallet Object get failed: ' . $ex->getMessage());
                return "{$issuerId}.{$objectSuffix}";
        ...
        $message = new \Google\Service\Walletobjects\AddMessageRequest([
            'message' => new \Google\Service\Walletobjects\Message([
                'header' => $header,
                'body' => $body
            ])
        ]);
  ...
  // [START jwtExisting]
     ...
     * @return string An "Add to Google Wallet" link.
     */
    public function createJwtExistingObjects(string $issuerId, string $classSuffix, string $objectSuffix)
    {
  ...
  // [START batch]
    ...
    public function batchCreateObjects(string $issuerId, string $classSuffix)
    {
        ...
        foreach ($batchResponse as $key => $value) {
            if ($value instanceof \Google\Service\Exception) {
